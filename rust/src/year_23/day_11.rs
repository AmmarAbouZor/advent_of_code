...
fn get_expanded(input: &str) -> Vec<Galaxy> {
    ...
    for mut galaxy in galaxies {
        col_offset += galaxy.col.saturating_sub(last_col).saturating_sub(1);
        last_col = galaxy.col;
...
fn calc_paths_sum(input: &str) -> usize {
    let galaxies = get_expanded(input);
...
fn part_1(input: &str) {
    let answer = calc_paths_sum(input);
...
#...#.....";
    ...
    #[test]
    fn test_solution() {
        assert_eq!(calc_paths_sum(INPUT), 374);
    }
}
